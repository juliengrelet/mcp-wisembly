...
import { createErrorResponse, createSuccessResponse, formatResponse } from "../utils/formatResponse.js";
...
const getEvent = async ({ keyword }: GetEvent): Promise<CallToolResult> => {
  try {
    ...
    const formattedResponse: string = formatResponse(data, keyword);
